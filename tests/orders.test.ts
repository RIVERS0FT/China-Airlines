...
describe('save v2 and legacy migration',()=>{
  it('migrates a fixed v1 flight fixture without changing money or locked revenue',()=>{
    const c=new GameCore(NOW,legacy),s=c.snapshot();expect(s.version).toBe(4);expect(s.credits).toBe(legacy.credits);expect(s.fleet.map(({upgrades: _u,itinerary: _i,dispatcher: _d,...p})=>p)).toEqual(legacy.fleet);
