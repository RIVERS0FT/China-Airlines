...
describe('clock boundaries', () => {
  ...
  it.each([-1, NaN, Infinity, 8.64e15 + 1])('rejects invalid ticks without changing state: %s', now => {
    const core = routeWithTwoPlanes(), before = core.snapshot();
    expect(() => core.tick(now)).toThrow('无效的系统时间');expect(core.snapshot()).toEqual(before);
  });
});
describe('consistent flight state', () => {
  it('rejects departure before ground turnaround is complete', () => {
    const state = routeWithTwoPlanes().snapshot();state.fleet[0]!.readyAt = state.fleet[0]!.flight!.departAt + 1;
