...
function rememberRoute(s: GameState, from: string, to: string) {
...
export function planQuote(s: GameState, p: Plane, stops: string[]) {
  check(Array.isArray(stops) && stops.length > 0 && stops.length <= MAX_PLAN_LEGS, '运输路线需要 1 至 5 个航段');
...
function depart(s: GameState, p: Plane, to: string, auto: boolean) {
  ...
  const id = rememberRoute(s, p.airportId, to); p.autoRouteId = auto ? id : null;
  ...
  for (;;) {
    ...
    if (next.itinerary.length) {
      try {
        ...
        note(s, `${next.id} 运输路线停止：${error instanceof Error ? error.message : '无法起飞'}`);
      }
      continue;
    }
    const r = s.routes.find(r => r.id === next!.autoRouteId);
    try {
      check(r, '自动路线不存在'); const to = r!.from === next.airportId ? r!.to : r!.from;
  ...
  execute(command: Command, now: number) {
    ...
    switch (command.type) {
      ...
      case 'start-duty': {
        ...
        else { rememberRoute(s, p.airportId, command.to); p.autoRouteId = id; p.readyAt = s.nextDemandAt; }
      ...
      case 'open-plan-routes': {
        ...
        for (const leg of q.legs) rememberRoute(s, leg.from, leg.to);
        note(s, `${p.id} 已记录路线草稿 · 城市解锁后无需建设费`); break;
      }
      case 'dispatch-plan': {
        ...
        note(s, `${p.id} 运输路线开始 · ${command.stops.map(id => airport(id).city).join(' → ')}`); break;
      }
      case 'cancel-plan': {
        const p = s.fleet.find(p => p.id === command.planeId); check(p, '未找到这架飞机');
        check(p!.itinerary.length, '没有剩余运输路线'); p!.itinerary = [];
        note(s, `${p!.id} 剩余路线已取消，当前航班不受影响`); break;
      }
      case 'load': {
      ...
      case 'route': {
        check(command.from !== command.to, '路线需要两个不同机场'); check(owned(s, command.from) && owned(s, command.to), '请先解锁两端机场');
        const id = routeId(command.from, command.to); check(!s.routes.some(r => r.id === id), '路线已经记录');
        rememberRoute(s, command.from, command.to); note(s, `记录${airport(command.from).city} ↔ ${airport(command.to).city}航迹`); break;
      }
      case 'dispatch': {
        const p = s.fleet.find(p => p.id === command.planeId); check(p, '未找到这架飞机'); check(typeof command.auto === 'boolean', '自动往返参数无效');
        check(!p!.itinerary.length, '请先取消剩余运输路线');
...
export function validateSave(value: unknown): GameState {
  ...
  for (const p of s.fleet) {
    ...
    if (p.itinerary.length) {
      ...
      planQuote(s, afterCurrent, p.itinerary);
    }
  }
  return s;
}
