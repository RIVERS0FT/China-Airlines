import { routeId } from './catalog.js';
...
export function guideStep(game: GameState, plane: Plane, screen: 'airport' | 'map', to: string): GuideStep {
  if (game.claimedTasks.includes('first-flight')) return { id: 'done', number: 7, title: '首航培训完成', text: '继续解锁机场、收集飞机与安排中转。引导不会额外发钱或重置进度。', target: '.game-dock', screen: 'airport' };
  ...
  if (!game.routes.some(r => r.id === routeId(plane.airportId, to))) return { id: 'route', number: 3, title: '开通首条航线', text: '选择订单的目的地，开通两端已解锁的航线。建设费只支付一次，往返共用。', target: '[data-guide="open-route"]', screen: 'map' };
  return { id: 'dispatch', number: 4, title: '核对后确认起飞', text: '检查用时、成本和本段交付收入，点击「确认起飞」。只在订单最终目的地付款。', target: '[data-testid="dispatch"]', screen: 'map' };
}
