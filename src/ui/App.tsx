...
export function App() {
  ...
  const [modal, setModal] = useState<'shop' | 'fleet' | 'tasks' | 'settings' | 'help' | 'airports' | 'airport-detail' | null>(null);
  ...
  function showMap(target?: string) {
    ...
    setDestination(target ?? (!plane && context.pinned ? context.pinned : to));
  ...
  return <>
    ...
    {game && modal && modal !== 'settings' && <Modal key={modal} title={{ shop: '飞机商店', fleet: '我的机库', tasks: '运营任务', help: '起航指南', airports: '机场目录', 'airport-detail': '机场详情' }[modal]} onClose={() => setModal(null)}>
      {modal === 'airports' ? <AirportDirectory game={game} onInspect={inspectAirport}/> : modal === 'airport-detail' ? <AirportDetails game={game} id={detailAirport} busy={view.busy} onBack={() => setModal('airports')} onVisit={visitAirport} onPlane={selectPlane} onRoute={id => showMap(id)}/> : modal === 'shop' ? <Shop game={game} busy={view.busy} selected={current}/> : modal === 'tasks' ? <Tasks game={game} busy={view.busy}/> : modal === 'fleet' ? <Hangar game={game} busy={view.busy} onSelect={selectPlane}/> : <div className="help-content"><button className="start-guide-help" disabled={view.busy} onClick={() => { ignore(act({type:'tutorial',action:'start'}).then(() => { setModal(null); setBrowsedAirport(null); setScreen('airport'); })); }}>开始分步引导</button><h3>装载、规划、起飞，经营你的航空网络。</h3><p>① 点击下方旅客或货物卡片装载；同目的地装载只取当前机场的真实订单，容量不足时跳过整单。</p><p>② 点击「选择航线起飞」，在地图选择目的地，开通航线后确认起飞。空机调机仍需支付成本。</p><p>③ 到达最终目的地才交付并付款；其他订单留在机上。周转结束后可卸至机场，再让另一架飞机装载中转。</p><p>④ 自动往返需随航调度员，仅运送直达订单；客源不足时等待补充，不会凭空产生旅客。已接受的订单不会过期。</p><p>点击「机场目录」查看各地客货、停靠及飞来航班，再进入候机大厅；没有停靠飞机时只能浏览，可安排飞机飞来或到商店购机交付。</p><p>使用左右箭头或机库切换飞机。每10秒和关键操作后保存；旧版存档自动迁移，仍建议定期导出。</p><small>机库支持扩建与四类改装；商店分纯客、纯货和客货机。多段计划最多5段，逐站交付，不在途中自动接新订单。机库可雇用调度员或出售卸空的飞机，至少保留一架。完整原版数值与美术仍需对照。</small></div>}
    </Modal>}
