...
import { AIRPORTS, airport, model, routeId, routePrice, upgradePrice } from '../core/catalog.js';
...
function OrderCard({ order, aboard, disabled, onClick }: { order: Order; aboard: boolean; disabled: boolean; onClick: () => void }) {
  return <button className={`job-card ${aboard ? 'aboard' : ''}`} disabled={disabled} onClick={onClick}
    ...
    <strong>{money(order.reward)}</strong><small className="job-state">{aboard ? '已装载 · 点击卸下' : order.expiresAt === null ? '中转保留 · 点击装载' : '点击装载'}</small>
  </button>;
}
function OrderBoard({ game, plane, airportId, aboard, setAboard, destination, setDestination, busy }: {
  ...
  const locked = busy || !plane || Boolean(plane.flight) || Boolean(plane.autoRouteId) || plane.readyAt > game.simTime;
  return <section className="order-board" aria-label="客货装载区">
    ...
    <div className="order-strip">{shown.map(o => <OrderCard key={o.id} order={o} aboard={aboard} disabled={locked} onClick={() => plane && ignore(act({ type: aboard ? 'unload' : 'load', planeId: plane.id, orderId: o.id }))}/>)}
...
function Network({ game, plane, destination, setDestination, onReturn, onDepart, busy }: {
  ...
  const inFlight = Boolean(plane?.flight), cooling = plane && game.simTime < plane.readyAt;
...
export function App() {
  ...
  const total = game && plane ? loadSummary(game, plane.id) : { passengers: 0, cargo: 0 }, m = plane ? model(plane.modelId) : null;
  const f = plane?.flight, cooling = plane && game && plane.readyAt > game.simTime;
  return <>
    <div className="aviation-game"><header className="game-hud"><div className="game-brand"><span className="pilot-badge"><Icon name="plane"/></span><div><h1>中华航空</h1><small>航空运输经营 · v0.2</small></div></div><div className="resource"><i>币</i><span><small>运营资金</small><strong data-testid="credits">{money(game?.credits ?? 0)}</strong></span></div><div className="resource"><Icon name="fleet"/><span><small>我的机队</small><strong data-testid="fleet-count">{game?.fleet.length ?? 0} 架</strong></span></div><div className="resource"><Icon name="check"/><span><small>完成航班</small><strong data-testid="flights-count">{game?.stats.flights ?? 0} 班</strong></span></div><button aria-label="操作帮助" onClick={() => setModal('help')}>?</button><button aria-label="存档设置" onClick={() => setModal('settings')}><Icon name="save"/><span>{view.busy ? '保存中' : view.error ? '需注意' : '已存档'}</span></button></header>
    ...
    {game && modal && modal !== 'settings' && <Modal title={{ shop: '飞机商店', fleet: '我的机库', tasks: '运营任务', help: '起航指南' }[modal]} onClose={() => setModal(null)}>
      {modal === 'shop' ? <Shop game={game} busy={view.busy} selected={current}/> : modal === 'tasks' ? <Tasks game={game} busy={view.busy}/> : modal === 'fleet' ? <div className="hangar-list">{game.fleet.map(p => <button key={p.id} onClick={() => selectPlane(p.id)}><Icon name="plane"/><strong>{model(p.modelId).name} · {p.id}</strong><span>{p.flight ? `${airport(p.flight.from).city} → ${airport(p.flight.to).city} · 飞行中` : `${airport(p.airportId).city} · 地面`}</span><small>机上 {manifest(game, p.id).length} 单 · 点击查看</small></button>)}</div> : <div className="help-content"><h3>装载、规划、起飞，经营你的航空网络。</h3><p>① 点击下方旅客或货物卡片装载；同目的地装载只取当前机场的真实订单，容量不足时跳过整单。</p><p>② 点击「选择航线起飞」，在地图选择目的地，开通航线后确认起飞。空机调机仍需支付成本。</p><p>③ 到达最终目的地才交付并付款；其他订单留在机上。周转结束后可卸至机场，再让另一架飞机装载中转。</p><p>④ 自动往返仅运送直达订单；客源不足时等待补充，不会凭空产生旅客。已接受的订单不会过期。</p><p>使用左右箭头或机库切换飞机。每10秒和关键操作后保存；旧版存档自动迁移，仍建议定期导出。</p><small>本轮是复刻方向的第一阶段。完整机型分类、改装养成、多段计划及原版数值对照尚未完成。</small></div>}
    </Modal>}
