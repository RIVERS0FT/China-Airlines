...
import { FlightBoard, FlightMoney } from './FlightBoard.js';
...
export function App() {
  ...
  const [modal, setModal] = useState<'career' | 'shop' | 'fleet' | 'tasks' | 'settings' | 'help' | 'airports' | 'airport-detail' | 'flights' | null>(null);
  ...
  return <>
    <div className={`aviation-game ${screen === 'map' ? 'dispatch-focused' : ''} ${screen !== 'map' && game?.tutorial === 'active' && plane && !modal ? 'training-active' : ''}`}><header className="game-hud" hidden={screen === 'map'}><div className="game-brand"><span className="pilot-badge"><Icon name="pilot"/></span><div><h1>中华航空</h1><small>公司 Lv.{game ? careerLevel(game) : 1} · 单机经营</small></div></div><div className="resource"><Icon name="coin"/><span><small>运营资金</small><strong data-testid="credits">{money(game?.credits ?? 0)}</strong></span></div><button className="resource fleet-status-button" aria-label="航班运行表" disabled={!game} onClick={() => setModal('flights')}><Icon name="fleet"/><span><small>机队运行表</small><strong data-testid="fleet-count">{game?.fleet.length ?? 0} 架</strong></span></button><button className="resource fleet-status-button" aria-label="经营中心" disabled={!game} onClick={() => setModal('career')}><Icon name="trophy"/><span><small>经营中心 · 点券</small><strong>{game?.career.tickets ?? 0} 券</strong><span className="sr-only" data-testid="flights-count">{game?.stats.flights ?? 0} 班</span></span></button><button aria-label="操作帮助" onClick={() => setModal('help')}><Icon name="help"/></button><button aria-label="存档设置" onClick={() => setModal('settings')}><Icon name="save"/><span>{view.busy ? '保存中' : view.error ? '需注意' : '已存档'}</span></button></header>
      ...
      {!game ? <main className="startup"><h2>{view.booting ? '正在准备机场…' : '进度尚未载入'}</h2><p>{view.error}</p><button onClick={() => setModal('settings')}>导入或恢复存档</button></main> : <main className="game-workspace">
        {screen === 'map' ? <Network key={plane?.id} onInspect={(id, afterUnlock) => inspectAirport(id, afterUnlock, 'map')} game={game} plane={plane} destination={destination} setDestination={setDestination} onReturn={returnFromMap} onDepart={() => { mapReturn.current = null; setScreen('airport'); setAboard(true); }} busy={view.busy}/> : <>
          ...
          <div className="scene-wrap">{context?.pinned && <button className="return-selected-plane" onClick={() => selectPlane(context.selected.id)}>返回所选飞机 {context.selected.id}</button>}{plane && plane.itinerary.length > 0 && <div className="itinerary-banner" data-testid="active-plan">后续航段：{plane.itinerary.map(id => airport(id).city).join(" → ")}</div>}<AviationScene game={game} plane={plane} onCabin={() => f ? setModal('flights') : showOrders(true)}/><AirportSceneTools game={game} plane={plane} hasPlane={Boolean(plane)} onTasks={() => setModal('tasks')} onFleet={() => setModal('fleet')} onFlights={() => setModal('flights')}/><button className="switch-plane previous" aria-label="上一架飞机" disabled={!context || context.choices.length < 2} onClick={() => cycle(-1)}>◀</button><button className="switch-plane next" aria-label="下一架飞机" disabled={!context || context.choices.length < 2} onClick={() => cycle(1)}>▶</button>
    ...
    {game && modal && modal !== 'settings' && <Modal key={modal} title={{ career: '公司经营中心', shop: '飞机商店', fleet: '我的机库', tasks: '运营任务', help: '起航指南', airports: '机场目录', 'airport-detail': '机场详情', flights: '航班运行表' }[modal]} onClose={closeModal}>
      {modal === 'career' ? <CareerHub game={game} busy={view.busy} selected={plane?.id ?? context?.selected.id} airportId={current}/> : modal === 'flights' ? <FlightBoard game={game} selectedId={plane?.id ?? context?.selected.id} onSelect={selectPlane}/> : modal === 'airports' ? <AirportDirectory game={game} onInspect={id => inspectAirport(id, undefined, 'directory')}/> : modal === 'airport-detail' ? <AirportDetails game={game} id={detailAirport} busy={view.busy} backLabel={detailBack === 'directory' ? '返回机场目录' : detailBack === 'map' ? '返回航线地图' : '返回机场装载'} onBack={() => { routeUnlockResume.current = null; if (detailBack === 'directory') setModal('airports'); else setModal(null); }} onUnlocked={() => { const resume = routeUnlockResume.current; routeUnlockResume.current = null; if (resume) { setModal(null); resume(); } }} onVisit={visitAirport} onPlane={selectPlane} onRoute={id => showMap(id)}/> : modal === 'shop' ? <Shop game={game} busy={view.busy} selected={current}/> : modal === 'tasks' ? <Tasks game={game} plane={plane ?? context?.selected} busy={view.busy} onNext={step => { if (step === 'map') showMap(); else if (step === 'flight') setModal('flights'); else { if (context) selectPlane((plane ?? context.selected).id); showOrders(false); } }}/> : modal === 'fleet' ? <Hangar game={game} busy={view.busy} selectedPlaneId={plane?.id ?? context?.selected.id} onSelect={selectPlane}/> : <div className="help-content"><button className="start-guide-help" disabled={view.busy} onClick={() => { ignore(act({type:'tutorial',action:'start'}).then(() => { setModal(null); setBrowsedAirport(null); setScreen('airport'); })); }}>开始分步引导</button><h3>装载、规划、起飞，经营你的航空网络。</h3><p>① 点击下方旅客或货物装载；点击目的地站牌可装载同站客货，只取当前机场的真实订单，容量不足时跳过整单。</p><p>② 点击「制定路线」，在地图依次点击已解锁城市，点击顺序就是飞行路线；点顶部目的地可打开城市列表，“查看路线”包含自动往返、收支与能量明细，“取消起飞”返回进入前场景；城市解锁后可直接前往，不需要另行开通航线；解锁和升级只在按需打开的机场详情里操作，不常驻地图。空机调机仍需支付运营成本。</p><p>③ 到达最终目的地才交付并付款；其他订单留在机上。周转结束后可卸至机场，再让另一架飞机装载中转。</p><p>④ 自动往返需随航调度员，仅支持一个目的城市并运送直达订单；客源不足时等待补充，不会凭空产生旅客。已接受的订单不会过期。</p><p>点击「机场目录」查看各地客货、停靠及飞来航班，再进入候机大厅；没有停靠飞机时只能浏览，可安排飞机飞来或到商店购机交付。</p><p>使用左右箭头或机库切换飞机。每10秒和关键操作后保存；旧版存档自动迁移，仍建议定期导出。</p><small>机库支持扩建与四类改装；商店分纯客、纯货和客货机。路线最多12段，逐站交付，不在途中自动接新订单；可撤销末段或清空后重选。机库可雇用调度员或出售卸空的飞机，至少保留一架。完整原版数值与美术仍需对照。</small></div>}
    </Modal>}
