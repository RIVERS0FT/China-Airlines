...
export function EnergyService({ game, plane, busy }: { game: GameState; plane: Plane; busy: boolean }) {
  ...
  return <section className="energy-service" aria-label="飞机能量管理">
    ...
    <small>下次补能 {service.seconds} 秒{service.employee ? `（${service.employee.name}负责，效率提升${service.reduction}%）` : "（基础地勤）"}，费用 0，已开始服务的截止时间锁定；完成前不增加能量；取消不补能。容量随机型与动力等级变化。营业日刷新时恢复电力；在途已预留电力不会重复赠送。</small>
  </section>;
}
