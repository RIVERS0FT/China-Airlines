...
export function PlanControls({ game, plane, stops, setStops, destination, onChoose, onInspect, auto, setAuto, busy, onDepart, onCancel }: {
  game: GameState; plane: Plane; stops: string[]; setStops: (v: string[]) => void;
  destination: string; onChoose: (id: string) => void; onInspect: () => void;
  auto: boolean; setAuto: (value: boolean) => void; busy: boolean; onDepart: () => void; onCancel: () => void;
}) {
  ...
  const summaryText = preview
    ? `${stops.length} 段 · ${duration(preview.duration)} · 成本 ${money(preview.cost)} · 交付 ${money(preview.revenue)} · 运输净收益 ${money(preview.profit)}`
    : error || `依次点击城市规划路线；最多 ${MAX_PLAN_LEGS} 段，每站周转 8 秒。`;
  const dispatchLabel = stops.length > 1 ? '确认路线起飞' : '确认起飞';
  return <section className="plan-editor route-dispatch-console" aria-label="路线规划">
    <div className="plan-console-info">
      ...
      <div className="plan-summary"><span data-testid="plan-summary">{summaryText}</span></div>
    ...
    <div className="plan-edit-actions route-console-actions" aria-label="路线编辑">
      <button className="route-console-button" aria-label="撤销末段" disabled={locked || !stops.length} onClick={() => setStops(stops.slice(0, -1))}><span aria-hidden="true">路线</span><strong aria-hidden="true">后退</strong></button>
      <button className="route-console-button" aria-label="清空路线" disabled={locked || !stops.length} onClick={() => setStops([])}><span aria-hidden="true">路线</span><strong aria-hidden="true">撤销</strong></button>
      <button className="route-console-button route-cancel-button" aria-label="取消路线规划" onClick={cancelPlanning}><span aria-hidden="true">取消</span><strong aria-hidden="true">起飞</strong></button>
      <button className="route-console-button route-dispatch-button gold-button" data-testid="dispatch" aria-label={dispatchLabel} disabled={locked || !preview || autoBlocked || Boolean(energyReason) || game.credits < (preview?.legs[0]?.cost ?? 0)} onClick={() => ignore(launch())}><span aria-hidden="true">起飞</span><strong aria-hidden="true">航线</strong></button>
    </div>
  </section>;
}