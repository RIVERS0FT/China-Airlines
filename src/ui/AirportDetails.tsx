...
export function AirportDetails({ game, id, busy, backLabel = '返回机场目录', onBack, onUnlocked, onVisit, onPlane, onRoute }: {
  ...
  return <section className="airport-details">
    <div className="airport-detail-hero"><span className="airport-code-badge">{a.id}</span><div><h3>{t('airport.name',{city:city(a.id)})}</h3><p>{airportRegion(a.id,a.region)} · {a.level ? ui('{value} 级机场',{value:a.level}) : ui('尚未开放')}</p></div><button onClick={onBack}>{backLabel === '返回机场目录' ? ui('返回机场目录') : backLabel}</button></div>
    ...
    {!a.level ? <p className="airport-empty">{ui('解锁后可浏览候机大厅、接收客货并购买飞机交付到此处。解锁不会把已有飞机移动到本机场。')}</p> : <>
      ...
      <div className="airport-detail-columns">
        <section><h4>{ui('候运去向')}</h4><ul className="airport-destinations">{a.destinations.map(d => <li key={d.id}><strong>{city(d.id)} · {d.id}</strong><span>{d.passengers} {ui('人')} / {d.cargo} {ui('吨')} · {d.orders} {ui('单')}{d.transfers > 0 ? ` (${ui('中转')} ${d.transfers} ${ui('单')})` : ''}</span></li>)}</ul>{!a.destinations.length && <p className="airport-empty">{ui('暂无候运客货。')}</p>}</section>
