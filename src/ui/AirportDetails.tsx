...
export function AirportDetails({ game, id, busy, onBack, onVisit, onPlane, onRoute }: {
  game: GameState; id: string; busy: boolean; onBack: () => void;
  ...
  return <section className="airport-details">
    <div className="airport-detail-hero"><span className="airport-code-badge">{a.id}</span><div><h3>{a.city}航空港</h3><p>{a.region} · {a.level ? `${a.level} 级机场` : '尚未开放'}</p></div><button onClick={onBack}>返回机场目录</button></div>
    <div className="airport-detail-actions">
      ...
      {a.level < 3 ? <button disabled={busy || game.credits < price} onClick={() => ignore(controller.command({ type: a.level ? 'upgrade' : 'unlock', airportId: id }))}>
