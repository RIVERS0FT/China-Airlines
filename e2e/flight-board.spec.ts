...
test('fleet filters switch scenes only and the workshop keeps the chosen aircraft',async({page})=>{
  const s=flying(),second=s.fleet[1]!.id;await load(page,s);await page.getByRole('button',{name:'航班运行表',exact:true}).click();
  ...
  await page.getByRole('button',{name:`查看${second}飞机`}).click();await expect(page.locator('.plane-status')).toContainText(second);
  ...
  await page.getByRole('button',{name:'机队管理',exact:true}).click();await expect(page.locator('.hangar-selector button[aria-pressed=true]')).toContainText(second);
  await page.getByRole('button',{name:'关闭我的机库'}).click();await page.getByRole('button',{name:'航班运行表',exact:true}).click();
  await page.getByRole('button',{name:/^飞行中的飞机/}).click();await page.getByRole('button',{name:`查看${ID}飞机`}).click();
...
test('a next leg appears only after departure and final delivery cannot be repeated by viewing',async({page})=>{
  ...
  await page.getByRole('button',{name:'航班运行表',exact:true}).click();await expect(page.locator(`[data-plane-id="${ID}"] .flight-route`)).toContainText('武汉 → 上海');
...
test('automatic waiting is not mislabeled as ready or a paid flight',async({page})=>{
  ...
  await page.getByRole('button',{name:'航班运行表',exact:true}).click();const row=page.locator(`[data-plane-id="${ID}"]`);
