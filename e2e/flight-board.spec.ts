...
test('fleet filters switch scenes only and the workshop keeps the chosen aircraft',async({page})=>{
  const s=flying(),second=s.fleet[1]!.id;await load(page,s);await openGlobal(page, '航班运行表');
  ...
  await page.getByRole('button',{name:'关闭我的机库'}).click();await openGlobal(page, '航班运行表');
...
test('automatic waiting is not mislabeled as ready or a paid flight',async({page})=>{
  ...
  await openGlobal(page, '航班运行表');const row=page.locator(`[data-plane-id="${ID}"]`);
