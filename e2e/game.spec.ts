...
async function ready(page:Page){await page.goto('./');await expect(page.getByTestId('fleet-count')).toHaveText('1 架');await expect(page.getByTestId('map-canvas')).toHaveAttribute('data-renderer','ready');}
async function settings(page:Page){await page.getByRole('button',{name:'存档设置',exact:true}).click();await expect(page.getByRole('dialog')).toBeVisible();}
test('desktop map, first flight, reward, and reload',async({page})=>{
  await page.clock.install({time:new Date('2026-09-11T00:00:00Z')});await ready(page);
  await page.getByLabel('自动往返').uncheck();await page.getByRole('button',{name:/开通航线/}).click();
  await expect(page.getByTestId('dispatch')).toBeEnabled();await page.getByTestId('dispatch').click();
  await expect(page.getByRole('region',{name:'在途航班'}).getByText('单程航班')).toBeVisible();
...
test('cached app starts offline without an external runtime dependency',async({page,context})=>{
  ...
  await expect(page.getByTestId('fleet-count')).toHaveText('1 架');await expect(page.getByTestId('map-canvas')).toHaveAttribute('data-renderer','ready');
  await page.getByRole('button',{name:'飞机商店',exact:true}).click();await page.getByRole('button',{name:'购买云雀 70',exact:true}).click();
...
test('landscape touch layout and portrait prompt',async({browser,baseURL})=>{
  ...
  await page.getByLabel('选择机场',{exact:true}).selectOption('WUH');await page.getByRole('button',{name:/解锁机场/}).click();
  await expect(page.getByText('已解锁 3',{exact:false})).toBeVisible();
...
test('captures map and collection screenshots with no overflow',async({page})=>{
  await ready(page);await page.screenshot({path:'artifacts/desktop-map.png',fullPage:true});
  await page.getByRole('button',{name:'飞机商店',exact:true}).click();await expect(page.getByRole('heading',{name:'下一架，为更远的航程。'})).toBeVisible();
  await page.screenshot({path:'artifacts/desktop-shop.png',fullPage:true});
