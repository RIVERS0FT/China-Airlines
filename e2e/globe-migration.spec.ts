...
import { GameCore } from '../src/core/v7/game.js';
const core=new GameCore(Date.parse('2026-09-12T00:00:00Z'));core.execute({type:'load-destination',planeId:'AC0001',to:'PVG'},core.snapshot().lastWallTime);core.execute({type:'dispatch',planeId:'AC0001',to:'PVG',auto:false},core.snapshot().lastWallTime);const unitFlying=core.snapshot();
import { leaveMap } from './dispatch-helpers.js';

for (const [width, height] of [[1440, 900], [667, 375]] as const) {
  test(`current v7 starter flight survives import, spherical inspection and v9 export at ${width}`, async ({ page }) => {
    ...
    await page.getByLabel('选择存档文件').setInputFiles({ name: 'v7-starter.json', mimeType: 'application/json', buffer: Buffer.from(JSON.stringify(unitFlying)) });
    ...
    await page.screenshot({ path: `artifacts/globe-v7-starter-${width}.png` });
    ...
    expect(saved.version).toBe(9); expect(saved.fleet).toEqual(unitFlying.fleet);
    expect(saved.orders).toEqual(unitFlying.orders); expect(saved.credits).toBe(unitFlying.credits);
    await page.getByRole('button', { name: '关闭存档设置', exact: true }).click();
    await page.reload(); await expect(page.getByTestId('passenger-capacity')).toHaveText('旅客 3 / 3 人');
