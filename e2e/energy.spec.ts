...
test('the exact flight budget is reserved once and survives reload and arrival',async({page})=>{
  ...
  await page.getByRole('button',{name:'航线地图',exact:true}).click();await page.getByLabel('选择机场',{exact:true}).selectOption('PVG');await page.getByTestId('dispatch').click();
...
test('multi-leg energy shortage stops at the hub without erasing transfer cargo',async({page})=>{
  ...
  await page.getByRole('button',{name:'确认路线起飞',exact:true}).click();await expect(page.getByTestId('plane-energy')).toHaveText('能量 0.00 点');
