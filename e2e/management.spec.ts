...
test('hangar duty can start for a reachable unlocked city without route purchase',async({page})=>{
  ...
  await page.getByRole('button',{name:'停止自动值勤',exact:true}).click();await expect(page.getByRole('button',{name:'人员下岗',exact:true})).toBeDisabled();await page.getByRole('button',{name:'关闭我的机库'}).click();
...
test('v3 automatic flight migrates without changed manifest or extra money',async({page})=>{
  ...
  expect(s.version).toBe(7);expect(s.orders.map(({service:_s,product:_p,...o}: import('../src/core/game.js').Order)=>o)).toEqual(legacy.orders);expect(s.fleet[0].flight).toEqual(legacy.fleet[0]!.flight);expect(s.fleet.every((p:{dispatcher:boolean})=>p.dispatcher)).toBe(true);expect(s.tutorial).toBe('skipped');
});
