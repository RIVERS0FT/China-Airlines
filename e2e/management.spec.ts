...
for(const width of [1440,844])test(`guided real first flight persists at ${width}`,async({page})=>{
  ...
  await page.getByRole('button',{name:'完成引导',exact:true}).click();await expect(page.getByTestId('tutorial')).toHaveCount(0);const credits=await page.getByTestId('credits').textContent();
  await page.reload();await expect(page.getByTestId('tutorial')).toHaveCount(0);await expect(page.getByTestId('credits')).toHaveText(credits!);expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
});
