...
async function secondPlane(page:Page){await openGlobal(page, '飞机商店');await page.getByRole('button',{name:'纯货机',exact:true}).click();await page.getByRole('button',{name:'购买云雀 8F',exact:true}).click();await expect(page.getByTestId('fleet-count')).toHaveText('2 架');await page.getByRole('button',{name:'关闭飞机商店'}).click();await openGlobal(page, '机队管理');await page.getByRole('button',{name:/云雀 8F.*AC0002/}).click();}
...
test('hangar duty can start for a reachable unlocked city without route purchase',async({page})=>{
  await ready(page);await openGlobal(page, '机队管理');
...
test('v3 automatic flight migrates without changed manifest or extra money',async({page})=>{
  ...
  expect(s.version).toBe(6);expect(s.orders).toEqual(legacy.orders);expect(s.fleet[0].flight).toEqual(legacy.fleet[0]!.flight);expect(s.fleet.every((p:{dispatcher:boolean})=>p.dispatcher)).toBe(true);expect(s.tutorial).toBe('skipped');
});
...
test('narrow landscape personnel controls and skip remain reachable',async({page})=>{
  ...
  await page.getByRole('button',{name:'关闭我的机库'}).click();await openGlobal(page, '操作帮助');await page.getByRole('button',{name:'开始分步引导',exact:true}).click();
