...
test('click-order route executes two legs, survives reload and does not pay twice',async({page})=>{
  ...
  await page.getByRole('button',{name:'确认路线起飞',exact:true}).click();await expect(page.getByTestId('active-plan')).toContainText('上海');
...
test('cancel plan in flight only removes onward destinations',async({page})=>{
  await ready(page);await plan(page);await page.getByRole('button',{name:'确认路线起飞',exact:true}).click();
