...
test('legacy import upgrades to v4 and exports real jobs with the old locked payment',async({page})=>{
  ...
  const file=await downloaded,save=JSON.parse(await readFile((await file.path())!,'utf8'));expect(save.version).toBe(8);expect(save.fleet[0].flight.revenue).toBe(legacy.fleet[0]!.flight!.revenue);expect(save.orders.filter((o:{location:string})=>o.location==='AC0001')).toHaveLength(2);
