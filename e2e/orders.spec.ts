...
test('manual loading, unloading, capacity, saved manifest, and plane switching',async({page},testInfo)=>{
  ...
  await expect(page.getByTestId('passenger-capacity')).toHaveText('旅客 3 / 3 人');await page.reload();await expect(page.getByTestId('passenger-capacity')).toHaveText('旅客 3 / 3 人');
  ...
  await page.getByRole('button',{name:'上一架飞机'}).click();await expect(page.getByTestId('passenger-capacity')).toHaveText('旅客 3 / 3 人');expect(errors).toEqual([]);
});
